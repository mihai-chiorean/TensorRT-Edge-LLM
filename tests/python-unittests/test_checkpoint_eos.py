...
def test_direct_builder_prefers_generation_config_stop_tokens(tmp_path):
    pytest.importorskip("torch")
    from experimental.builder.core.artifacts.runtime_artifacts import \
        _apply_generic_token_ids

    (tmp_path / "generation_config.json").write_text(
        json.dumps({"eos_token_id": [1, 106, 50]}))
    (tmp_path / "config.json").write_text(json.dumps({"eos_token_id": 7}))
    config = {}
    _apply_generic_token_ids(config, {"eos_token_id": [1, 106]}, str(tmp_path))
    assert config["eos_token_id"] == [1, 106, 50]
    ...
    config = {}
    _apply_generic_token_ids(config, {}, str(tmp_path))
    assert config["eos_token_id"] == [7]
