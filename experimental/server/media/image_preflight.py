...
from .media_source import MAX_IMAGE_SOURCE_BYTES, decode_base64_data_url
...
def preflight_image_data_url(url: str) -> Tuple[bytes, Tuple[int, int]]:
    ...
    return data, preflight_image_bytes(data)


def preflight_image_bytes(data: bytes) -> Tuple[int, int]:
    """Bound one encoded image already in memory; returns ``(width, height)``."""
