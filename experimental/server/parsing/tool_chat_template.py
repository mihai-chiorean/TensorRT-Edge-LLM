...
def _inject_tool_requirement(
    ...
    ``tool_choice`` Jinja variable, which makes ``required`` silently
    advisory. State the constraint at system priority without mutating
    ...
    def format(
        ...
        *,
        ...
        normalized_messages = _inject_tool_requirement(
            normalize_messages_for_tools(messages), tool_choice)
