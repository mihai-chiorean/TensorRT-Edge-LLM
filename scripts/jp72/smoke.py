...
t0 = time.time()
with post("/v1/chat/completions", {"model": MODEL, "messages": [
        {"role": "user", "content": "What sound does a cow make? Answer in one short sentence."}],
        "max_tokens": 64, "temperature": 0.0}) as r:
    out = json.load(r)
...
r = post("/v1/chat/completions", {"model": MODEL, "messages": [
        {"role": "user", "content": "Count from one to ten in words, separated by commas."}],
...
for raw in r:
    ...
    if payload == "[DONE]":
        arrivals.append((time.time()-t0, "[DONE]")); break
    obj = json.loads(payload)
    ch = obj.get("choices") or [{}]
    d = (ch[0].get("delta") or {}).get("content")
    if d:
        deltas.append(d); arrivals.append((time.time()-t0, d))
...
print("TTFT_stream: %.3fs   total: %.3fs" % (arrivals[0][0] if arrivals else -1, time.time()-t0))
...
if IMG:
    ...
    t0 = time.time()
    with post("/v1/chat/completions", {"model": MODEL, "messages": [{"role": "user", "content": [
            {"type": "text", "text": "What do you see? Answer in two short sentences."},
            {"type": "image_url", "image_url": {"url": "data:image/jpeg;base64," + b64}}]}],
            "max_tokens": 192, "temperature": 0.0}) as r:
        out = json.load(r)
    print("elapsed %.2fs" % (time.time()-t0))
